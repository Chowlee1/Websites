...
require_once "assets/app/db_con.php";
    ...
    if (isset($_GET['q'])) {
        ...
        if (mysqli_num_rows($query) < 1) {
            header("Location: all_articles.php?q=$search");
        }
    } else {
        ...
        if (!isset($_GET['s']) && !isset($_GET['e'])) {
            ...
            $sql = "SELECT * FROM user_posts WHERE post_status = '1' ORDER BY date_created DESC LIMIT $start,$end";
            $query = mysqli_query($connectDB, $sql);
        } else {
            $start = $_GET['s'];
            $end = $_GET['e'];
...
<body class="pattern-cross-dots-sm" style="background-color: #B3E6B5 ;">
    ...
    <div class="container my-5 bg-transparent">
        <div class="card mx-auto shadow bg-transparent">
            <div class="card-body bg-transparent">
                <?php
                if (!isset($_GET['q'])) {
                    $name = "STORIES"; ?>

                    <h2 class="text-center my-4">LATEST <?php echo $name; ?></h2>
               <?php } else {
                    if ($_GET['q'] < 1) {
                        $name = "Showing all stories";
                    }else{
                ...
                <div class="row pt-3 bg-transparent">
